...
elevations = dbGetQuery(db,"select [Plot.St], [Elevation.m.] as Elevation from UL_Info")
...
t_2016 = merge(t_2016, elevations, by=c("Plot.St"))

aelevation = data.frame(Plot.St=aoverstory$Plot.St, Elevation=aoverstory$Elevation)
aelevation = unique(aelevation)
...
t_1981 = merge(t_1981, aelevation, by=c("Plot.St"))
t_1981$Elevation = t_1981$Elevation*.3048
...
stani = subset(all_data, substr(Plot.St, 1,2) != "TR" & substr(Plot.St, 1,2) != "GP" )
yose = subset(all_data, substr(Plot.St, 1,2) != "HC" & substr(Plot.St, 1,2) != "CT" )

rf = subset(all_data, Species=="Abies magnifica")
wf = subset(all_data, Species=="Abies concolor")

ggplot(data=rf, aes(x=Elevation, y=ratio, group=Year, colour=Year)) +
    geom_point() +
    geom_smooth(method = "lm") + 
    ggtitle("Percent of population dominated by Abies magnifica by sampled year") 
...
ggplot(data=rf, aes(x=Elevation, y=ratio, group=Year, colour=Year)) +
    ...
    ggtitle("Percent of plot dominated by Abies magnifica by size class and elevation") 

ggplot(data=wf, aes(x=Elevation, y=ratio, group=Year, colour=Year)) +
    geom_point() +
    geom_smooth(method = "lm") + 
    facet_grid(~SizeClass) + 
    ggtitle("Percent of plot dominated by Abies concolor by size class and elevation") 


yose_upper = subset(yose, Band !=2)
agg = aggregate(data=yose_upper, avgperHA.x~Band+Species+Year, FUN=sum)


ggplot(data = agg, aes(x = Band)) + 
    geom_bar(aes(y=avgperHA.x, fill=factor(Species)),stat = "identity", position="fill")+
    xlab("Elevation Band (m)")+
    ylab("Percent") +
    ggtitle("Dominance of Abies concolor and magnifica across elevation bands in Yosemite NP") + 
    facet_grid(~Year) +
    coord_flip() + 
