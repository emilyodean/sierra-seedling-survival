...
seedlingsums = as.data.frame(aggregate(seedlings$Count~seedlings$Band + seedlings$Species, FUN=sum))
seedlingsums$SizeClass=0
names(seedlingsums) = c("Band", "Species", "Freq", "SizeClass")
...
overstory = as.data.frame(table(Band=overstory$Band, Species=overstory$Species, SizeClass=overstory$Size.Class))

#Make one dataframe with overstory and understory
seedlingsums$avgperHA = (seedlingsums$Freq/3)*100
overstory$avgperHA = (overstory$Freq/3)*10
...
#Proportion in each size class by species & year
