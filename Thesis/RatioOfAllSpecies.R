...
seedlings = dbGetQuery(db,"select * from Regeneration where Species !='?'")
...
overstory = dbGetQuery(db,"select * from Overstory where Species !='-'")
...
all_data = subset(all_data, Species!="no trees" & Species!="PD")
all_data[all_data == "PM"] = "WWP"
all_data[all_data == "QA"] = "BO"

agg = aggregate(data = all_data, avgperHA.x~Band+Species+Year, FUN=sum)
agg = subset(agg, Band!=1)
ggplot(data = agg, aes(x = Band)) + 
    geom_bar(aes(y=avgperHA.x, fill=factor(Species)),stat = "identity", position="fill")+
    xlab("Elevation Band (m)")+
    ylab("Percent") +
    ggtitle("Percent of total individuals by species and year") + 
    facet_grid(~Year) +
    scale_x_discrete(labels=getAllElevationBands()) +
    scale_y_continuous(labels=scales::percent) + 
    scale_fill_discrete(name="Species", labels=c("Quercus kellogii", 
...
levels(all_data$SizeClass) = c("Seedling", "Sapling", "10-20cm DBH", "20-40cm DBH", ">40cm DBH")
agg = aggregate(data = all_data, avgperHA.x~Band+Species+Year+SizeClass, FUN=sum)
agg = subset(agg, Band!=1)
ggplot(data = agg, aes(x = Band)) + 
    geom_bar(aes(y=avgperHA.x, fill=factor(Species)),stat = "identity", position="fill")+
    xlab("Elevation Band (m)")+
    ylab("Percent") +
    ggtitle("Percent of total individuals by size class, species and year") + 
...
#Just RFWF
rfwf = subset(all_data, Species=="RF" || Species=="WF")
agg = aggregate(data = all_data, avgperHA.x~Band+Species+Year, FUN=sum)
agg = subset(agg, Band!=1 & Band!=2)
ggplot(data = agg, aes(x = Band)) + 
    geom_bar(aes(y=avgperHA.x, fill=factor(Species)),stat = "identity", position="fill")+
    xlab("Elevation Band (m)")+
    ylab("Percent") +
